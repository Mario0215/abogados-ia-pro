...
export default function ClienteLogin() {
  ...
  return (
    <>
      <Head><title>Iniciar sesión — Portal del Cliente</title></Head>
      ...
      <div style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column' }}>
        ...
        <header style={{ background: '#ffffff', borderBottom: '1px solid #e5e5e5', padding: '0 24px', height: 56, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          ...
          <Link href="/cliente/registro" style={{ fontSize: 13, color: '#6b6b6b', textDecoration: 'none' }}>Crear cuenta</Link>
        </header>

        <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '40px 24px' }}>
          <div style={{ background: '#ffffff', border: '1px solid #e5e5e5', borderRadius: 16, padding: '40px 44px', width: '100%', maxWidth: 420 }}>
            {/* Marca */}
            <div style={{ textAlign: 'center', marginBottom: 32 }}>
              <div style={{ fontSize: 11, color: G, letterSpacing: 3, textTransform: 'uppercase', marginBottom: 8 }}>Portal del cliente</div>
              <h1 style={{ fontSize: 22, fontWeight: 700, color: '#3d3d3d' }}>Iniciar sesión</h1>
            </div>

            <form onSubmit={submit}>
              <div style={{ marginBottom: 16 }}>
                <label style={labelSt}>Correo electrónico</label>
                <input type="email" value={email} onChange={e => setEmail(e.target.value)} required autoFocus style={inputSt} placeholder="tu@correo.com" />
              </div>
              <div style={{ marginBottom: 24 }}>
                <label style={labelSt}>Contraseña</label>
                <input type="password" value={password} onChange={e => setPassword(e.target.value)} required style={inputSt} placeholder="••••••••" />
              </div>
            ...
            <div style={{ textAlign: 'center', marginTop: 24, fontSize: 13, color: '#9b9b9b' }}>
              ¿No tienes cuenta?{' '}
              <Link href="/cliente/registro" style={{ color: G, textDecoration: 'none', fontWeight: 600 }}>Regístrate</Link>
            </div>
          </div>
        </div>
